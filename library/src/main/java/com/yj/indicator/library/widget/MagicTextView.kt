package com.yj.indicator.library.widget
...
import com.yj.indicator.library.R

import java.util.ArrayList
import java.util.WeakHashMap

class MagicTextView : android.support.v7.widget.AppCompatTextView {
    private val outerShadows: MutableList<Shadow> by lazy {
        mutableListOf<Shadow>()
    }
    private val innerShadows:  MutableList<Shadow> by lazy {
        mutableListOf<Shadow>()
    }
    private val canvasStore: WeakHashMap<String, Pair<Canvas, Bitmap>> by lazy {
        WeakHashMap<String, Pair<Canvas, Bitmap>>()
    }
    ...
    private fun init(attrs: AttributeSet?) {
        if (attrs != null) {
            ...
            if (a.hasValue(R.styleable.MagicTextView_mtv_foreground)) {
                val foreground = a.getDrawable(R.styleable.MagicTextView_mtv_foreground)
                if (foreground != null) {
                    this.setForegroundDrawable(foreground)
                } else {
                    this.setTextColor(a.getColor(R.styleable.MagicTextView_mtv_foreground, -0x1000000))
                }
            }

            if (a.hasValue(R.styleable.MagicTextView_mtv_background)) {
                val background = a.getDrawable(R.styleable.MagicTextView_mtv_background)
                if (background != null) {
                    this.setBackgroundDrawable(background)
                } else {
                    this.setBackgroundColor(a.getColor(R.styleable.MagicTextView_mtv_background, -0x1000000))
                }
            }
